...
//
// Ганяються напряму проти src/server.ts — Node 24 стрипає типи сам, збірка
// (tsc) не потрібна. Правиш .ts — просто запускай npm test.
...
const ENTRY = process.env.HW_ENTRY ?? 'src/server.ts';
